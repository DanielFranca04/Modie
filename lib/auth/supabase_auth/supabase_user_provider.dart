...
class ModieSupabaseUser extends BaseAuthUser {
  ModieSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> modieSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ModieSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
